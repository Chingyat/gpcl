...
template <typename T> struct is_offset_ptr : std::false_type {};

template <typename T> struct is_offset_ptr<offset_ptr<T>> : std::true_type {};

namespace detail {
template <typename T, typename = void> class offset_ptr_base {
public:
  typedef T &reference;
};

template <typename T>
class offset_ptr_base<T,
...
template <typename T> std::intptr_t as_integer(T *ptr) {
...
template <typename T> class offset_ptr : public detail::offset_ptr_base<T> {
private:
...
public:
  ...
  GPCL_DECL_INLINE constexpr static offset_type diff_to_offset(
      difference_type diff);

  /// Converts offset to pointer difference
  GPCL_DECL_INLINE constexpr static difference_type offset_to_diff(
      offset_type offset);
  ...
  template <typename U = T,
      typename std::enable_if<!std::is_void<U>::value &&
                                  !detail::is_unbounded_array<U>::value,
          int>::type = 0>
  friend GPCL_DECL_INLINE offset_ptr operator+(
      difference_type diff, const offset_ptr &ptr) {
    return ptr + diff;
  }
  ...
  friend GPCL_DECL_INLINE constexpr bool operator<(
      const offset_ptr &x, const offset_ptr &y) {
